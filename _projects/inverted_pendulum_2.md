...
Another thing worth noting is that we chose the gain matrix $$K$$ and the observer gain matrix rather arbitrarily by trial and error until they satisfied the required disturbance rejection settling time and damping ratio (a method called pole placement). A more systematic approach to choosing $$K$$ and $$L$$ would be to implement a LQG (Linear Quadratic Gaussian) controller. In LQG, instead of choosing $$K$$ directly, you choose the penalty weights for state deviation and control input, which are more intuitive than choosing $$K$$ directly via pole placement. LQG also finds the optimal observer matrix $$L$$ that minimizes the error covariance of the state estimate, eliminating the need for any arbitrary guessing.
